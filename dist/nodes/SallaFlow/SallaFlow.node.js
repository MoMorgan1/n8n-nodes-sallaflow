...
exports.logicalRequestId = logicalRequestId;
exports.normalizeInventoryItems = normalizeInventoryItems;
exports.normalizeSallaError = normalizeSallaError;
exports.parseJsonArray = parseJsonArray;
exports.parseJsonInput = parseJsonInput;
exports.parseJsonObject = parseJsonObject;
exports.readTelemetryHeaders = readTelemetryHeaders;
exports.withReadTelemetry = withReadTelemetry;
const n8n_workflow_1 = require("n8n-workflow");
const API = 'https://api.sallaflow.cloud';
function asDataObject(value) {
    return value;
}
function asNodeParameters(value) {
    return value;
}
function asNodeParameterArray(value) {
    return value;
}
function asRequestError(value) {
    return value;
}
// ── Helper: parse "1,2,3" into [1,2,3] or ["1","2","3"] ──
function csvToArray(val, asNumber = false) {
...
function parseJsonInput(raw, node, itemIndex, label) {
    ...
    try {
        ...
        throw new n8n_workflow_1.NodeOperationError(node, `${label} is not valid JSON. If you are using an n8n expression, make sure it starts with "=".`, { itemIndex });
    }
}
function normalizeInventoryItems(raw, node, itemIndex) {
    ...
    if (!Array.isArray(source) || source.length === 0)
        throw new n8n_workflow_1.NodeOperationError(node, 'Inventory update requires at least one product or variant entry.', { itemIndex });
    const allowedTypes = new Set(['id', 'sku', 'variant_id']);
    const allowedModes = new Set(['increment', 'decrement', 'overwrite']);
    return source.map((entry, index) => {
        var _a, _b;
        if (!entry || typeof entry !== 'object' || Array.isArray(entry))
            throw new n8n_workflow_1.NodeOperationError(node, `Inventory entry ${index + 1} must be an object.`, { itemIndex });
        ...
        const identiferType = (_a = item.identifer_type) !== null && _a !== void 0 ? _a : item.identifier_type;
        const identifer = (_b = item.identifer) !== null && _b !== void 0 ? _b : item.identifier;
        const mode = item.mode;
        const quantity = Number(item.quantity);
        if (!allowedTypes.has(String(identiferType || '')))
            throw new n8n_workflow_1.NodeOperationError(node, `Inventory entry ${index + 1}: identifier type must be id, sku, or variant_id.`, { itemIndex });
        if (identifer === undefined || identifer === null || String(identifer).trim() === '')
            throw new n8n_workflow_1.NodeOperationError(node, `Inventory entry ${index + 1}: identifier is required.`, { itemIndex });
        if (!Number.isFinite(quantity) || quantity < 0)
            throw new n8n_workflow_1.NodeOperationError(node, `Inventory entry ${index + 1}: quantity must be a number greater than or equal to 0.`, { itemIndex });
        if (!allowedModes.has(String(mode || '')))
            throw new n8n_workflow_1.NodeOperationError(node, `Inventory entry ${index + 1}: mode must be increment, decrement, or overwrite.`, { itemIndex });
        const normalized = {
...
function normalizeSallaError(requestError, retryStatuses = new Set()) {
    var _a, _b, _c, _d, _e;
    const reqErr = asRequestError(requestError);
    const body = ((_a = reqErr.response) === null || _a === void 0 ? void 0 : _a.data)
        || ((_b = reqErr.response) === null || _b === void 0 ? void 0 : _b.body)
        || ((_d = (_c = reqErr.cause) === null || _c === void 0 ? void 0 : _c.response) === null || _d === void 0 ? void 0 : _d.data)
        || reqErr.body;
    const status = ((_e = reqErr.response) === null || _e === void 0 ? void 0 : _e.status) || reqErr.statusCode || reqErr.httpCode;
    ...
    if (body) {
        ...
        const nestedErrorObject = asDataObject(nestedError !== null && nestedError !== void 0 ? nestedError : {});
        code = String(data.code || (typeof nestedError === 'object' ? nestedErrorObject.code : '') || '');
        fields = asDataObject(data.fields
            || data.errors
...
function parseJsonObject(value, label, node, itemIndex) {
    let parsed = value;
    if (typeof value === 'string') {
        const trimmed = value.trim();
        if (!trimmed) {
            throw new n8n_workflow_1.NodeOperationError(node, `${label} cannot be empty. Enter a JSON object such as {}.`, { itemIndex });
        }
        try {
            parsed = JSON.parse(trimmed);
        }
        catch (error) {
            throw new n8n_workflow_1.NodeOperationError(node, `${label} is not valid JSON: ${error instanceof Error ? error.message : 'check commas, quotes, and brackets'}.`, { itemIndex });
        }
    }
    if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed)) {
        throw new n8n_workflow_1.NodeOperationError(node, `${label} must be a JSON object, not an array or scalar value.`, { itemIndex });
    }
    return asDataObject(parsed);
}
function parseJsonArray(value, label, node, itemIndex) {
    let parsed = value;
    if (typeof value === 'string') {
        try {
    ...
    if (!Array.isArray(parsed)) {
        throw new n8n_workflow_1.NodeOperationError(node, `${label} must be a JSON array, for example [].`, { itemIndex });
    }
    return parsed;
}
...
function logicalRequestId() {
    return `${Date.now().toString(36)}-${(0, n8n_workflow_1.randomString)(24)}`;
}
function readTelemetryHeaders(context) {
    return {
        'X-SallaFlow-Logical-Request-Id': logicalRequestId(),
        'X-SallaFlow-Read-Context': context,
    };
}
function withReadTelemetry(options, context) {
...
async function fetchPaginated(context, endpoint, perPage = 60) {
    const pageSize = Math.max(1, Math.min(60, Number(perPage) || 60));
    const maxPages = 3;
    const data = [];
    for (let page = 1; page <= maxPages; page++) {
        const resp = await context.helpers.httpRequestWithAuthentication.call(context, 'sallaFlowApi', {
            method: 'GET',
        ...
        if (pageData.length === 0 || page >= totalPages)
            break;
    }
    return data;
}
class SallaFlow {
    constructor() {
        this.description = {
            ...
            inputs: [n8n_workflow_1.NodeConnectionTypes.Main], outputs: [n8n_workflow_1.NodeConnectionTypes.Main],
            usableAsTool: true,
            ...
            properties: [
                ...
                { displayName: 'Additional Fields', name: 'orderAdditionalFields', type: 'collection', placeholder: 'Add Field', default: {},
                    ...
                    options: [
                        { displayName: 'Additional Products (JSON)', name: 'extra_products', type: 'json', default: '[]',
                            description: 'Additional products as JSON array: [{"identifier_type":"id","identifier":123,"quantity":2,"options":[{"id":123,"value":["456"]}]}]' },
                ...
                { displayName: 'Option IDs JSON', name: 'deleteJsonBody', type: 'json', default: '[]',
                    ...
                    description: 'Array of option IDs to delete. Accepts raw IDs or objects with "id". Example: [123, 456, 789] or [{"id":123},{"id":456}]' },
                { displayName: 'Option Name or ID', name: 'optionId', type: 'options', required: true, default: '',
                    typeOptions: { loadOptionsMethod: 'getOptionsForSelectedProduct', loadOptionsDependsOn: ['productId'] },
                ...
                { displayName: 'Value JSON (Object or Array)', name: 'valueJsonBody', type: 'json', default: '[]',
                    ...
                    description: 'Single object or array of objects. Each object must include "id". Any of name, price, display_value, is_default can be set — omitted fields are preserved. Use Product Variant for SKU stock. Example bulk: [{"id":123,"price":5},{"id":456,"name":"Large"}].' },
                ...
                { displayName: 'Option JSON (Object or Array)', name: 'optionJsonBodyUpdate', type: 'json', default: '{}',
                    ...
                    description: 'JSON for the updated option(s). A single object updates the option picked above. An array updates many options — each entry must include an "id" field. Example single: {"name":"Size","required":true}. Example bulk: [{"id":123,"name":"Size"},{"id":456,"required":true}]. Each bulk entry uses its own safe read-merge-update request.' },
                ...
                { displayName: 'Category Name', name: 'categoryName', type: 'string', required: true, default: '',
                    ...
                    description: 'The category name, e.g. "Electronics" or "ملابس"' },
            ...
            loadOptions: {
                async getAbandonedCarts() {
                    try {
                        ...
                        return data.map((cart) => {
                            var _a, _b, _c, _d, _e;
                            const customerData = asDataObject((_a = cart.customer) !== null && _a !== void 0 ? _a : {});
                            const cartTotal = asDataObject((_b = cart.total) !== null && _b !== void 0 ? _b : {});
                            const amounts = asDataObject((_c = cart.amounts) !== null && _c !== void 0 ? _c : {});
                            const amountsTotal = asDataObject((_d = amounts.total) !== null && _d !== void 0 ? _d : {});
                            const customer = customerData.full_name
                                || [customerData.first_name, customerData.last_name].filter(Boolean).join(' ')
                                || customerData.mobile
                                || 'Unknown customer';
                            const total = (_e = cartTotal.amount) !== null && _e !== void 0 ? _e : amountsTotal.amount;
                        ...
                        return [{ name: `⚠ Could not load carts: ${(error === null || error === void 0 ? void 0 : error.message) || 'check the SallaFlow credential and carts.read scope'}`, value: '' }];
                    }
                },
                async getOrders() {
                    try {
                        ...
                        return data.map((o) => {
                            var _a, _b, _c, _d;
                            const customer = asDataObject((_a = o.customer) !== null && _a !== void 0 ? _a : {});
                            const amounts = asDataObject((_b = o.amounts) !== null && _b !== void 0 ? _b : {});
                            const amountsTotal = asDataObject((_c = amounts.total) !== null && _c !== void 0 ? _c : {});
                            const total = asDataObject((_d = o.total) !== null && _d !== void 0 ? _d : {});
                            return {
                        ...
                        return [{ name: `⚠ Could not load orders: ${(error === null || error === void 0 ? void 0 : error.message) || 'check the SallaFlow credential and orders.read scope'}`, value: '' }];
                    }
                },
                async getProducts() {
                    try {
                        ...
                        return data.map((p) => {
                            var _a;
                            const price = asDataObject((_a = p.price) !== null && _a !== void 0 ? _a : {});
                            return {
                ...
                async getVariantsForSelectedProduct() {
                    try {
                        ...
                        return data.map((variant) => {
                            var _a, _b, _c;
                            const price = asDataObject((_a = variant.price) !== null && _a !== void 0 ? _a : {});
                            const optionValues = Array.isArray(variant.related_option_values)
                                ? `options ${variant.related_option_values.join('/')}`
                                : 'variant';
                            return {
                                name: `${variant.sku || optionValues} — qty ${(_b = variant.stock_quantity) !== null && _b !== void 0 ? _b : '?'} (${(_c = price.amount) !== null && _c !== void 0 ? _c : '?'} ${price.currency || 'SAR'})`,
                                value: String(variant.id),
                            };
                        });
                    }
                    catch (err) {
                        return [{ name: `⚠ Error loading variants: ${(err === null || err === void 0 ? void 0 : err.message) || 'check credentials'}`, value: '' }];
                    }
                },
                async getCustomers() {
                    try {
                        ...
                        return [{ name: `⚠ Could not load customers: ${(error === null || error === void 0 ? void 0 : error.message) || 'check the SallaFlow credential and customers.read scope'}`, value: '' }];
                    }
                },
                async getCoupons() {
                    try {
                        ...
                        return data.map((c) => {
                            var _a;
                            const amount = asDataObject((_a = c.amount) !== null && _a !== void 0 ? _a : {});
                            return {
                        ...
                        return [{ name: `⚠ Could not load coupons: ${(error === null || error === void 0 ? void 0 : error.message) || 'check the SallaFlow credential and marketing.read scope'}`, value: '' }];
                    }
                },
                async getOrderStatuses() {
                    try {
                        const resp = await this.helpers.httpRequestWithAuthentication.call(this, 'sallaFlowApi', {
                            method: 'GET',
                ...
                async getCanceledOrderStatuses() {
                    try {
                        ...
                        const data = Array.isArray(resp.data) ? resp.data : [];
                        const canceled = data.filter((status) => (/cancel|إلغاء|ملغ/i.test(`${status.slug || ''} ${status.name || ''}`)));
                        ...
                        return [{ name: `⚠ Could not load canceled statuses: ${(error === null || error === void 0 ? void 0 : error.message) || 'check orders.read scope'}`, value: '' }];
                    }
                },
                async getCategories() {
                    try {
                        ...
                        return [{ name: `⚠ Could not load categories: ${(error === null || error === void 0 ? void 0 : error.message) || 'check the SallaFlow credential and categories.read scope'}`, value: '' }];
                    }
                },
                ...
                async getBrands() {
                    try {
                        ...
                        return [{ name: `⚠ Could not load brands: ${(error === null || error === void 0 ? void 0 : error.message) || 'check the SallaFlow credential and brands.read scope'}`, value: '' }];
                    }
                },
                async getValuesForSelectedOption() {
                    ...
                    try {
                        ...
                        if (!optionId)
                            return [{ name: '⚠ Pick an Option First', value: '' }];
                        const resp = await this.helpers.httpRequestWithAuthentication.call(this, 'sallaFlowApi', {
                            method: 'GET',
                            url: `${API}/api/v1/salla/products/options/${optionId}`,
                            headers: readTelemetryHeaders('dynamic-loader'),
                        });
                        const responseData = asDataObject((_a = resp.data) !== null && _a !== void 0 ? _a : {});
                        ...
                        return vals.map((v) => {
                            var _a, _b;
                            const priceObject = asDataObject((_a = v.price) !== null && _a !== void 0 ? _a : {});
                            const price = (_b = priceObject.amount) !== null && _b !== void 0 ? _b : v.price;
                        ...
                        return [{ name: `⚠ Error loading values: ${(err === null || err === void 0 ? void 0 : err.message) || 'unknown'}`, value: '' }];
                    }
                },
                async getOptionsForSelectedProduct() {
                    ...
                    try {
                        ...
                        return [{ name: `⚠ Error loading options: ${(err === null || err === void 0 ? void 0 : err.message) || 'unknown'}`, value: '' }];
                    }
                },
            },
        };
    ...
    async execute() {
        ...
        const extractError = (reqErr) => normalizeSallaError(reqErr);
        const sallaRequest = async (opts, ctx) => {
            ...
            try {
                const requestOptions = withReadTelemetry(opts, String((ctx === null || ctx === void 0 ? void 0 : ctx.readContext) || 'action'));
                const out = await helpers.httpRequestWithAuthentication.call(this, 'sallaFlowApi', requestOptions);
                lastCallAt = Date.now();
                return out;
            }
            catch (reqErr) {
                ...
                throw new n8n_workflow_1.NodeOperationError(node, `${prefix}${info.msg}`, {
                    itemIndex: ctx === null || ctx === void 0 ? void 0 : ctx.itemIndex,
                    description: Object.keys(info.fields).length ? JSON.stringify(info.fields) : undefined,
                });
            }
        };
        const requireValue = (value, label, itemIndex) => {
            const normalized = String(value !== null && value !== void 0 ? value : '').trim();
            if (!normalized) {
                throw new n8n_workflow_1.NodeOperationError(node, `${label} is required. Select a value or use an expression that resolves to an ID.`, { itemIndex });
            }
            return normalized;
        };
        const validateDate = (value, label, itemIndex) => {
            if (!value)
                return;
            if (!/^\d{4}-\d{2}-\d{2}$/.test(String(value))) {
                throw new n8n_workflow_1.NodeOperationError(node, `${label} must use YYYY-MM-DD format, for example 2026-07-28.`, { itemIndex });
            }
        };
        const appendFilters = (targetUrl, filters, fields, itemIndex) => {
            for (const field of fields) {
                const value = filters === null || filters === void 0 ? void 0 : filters[field];
                if (value === undefined || value === null || value === '')
                    continue;
                if (field.includes('date')) {
                    const values = String(value).split(',').map((part) => part.trim()).filter(Boolean);
                    for (const part of values)
                        validateDate(part, field.replace(/_/g, ' '), itemIndex);
                }
                targetUrl += `&${field}=${encodeURIComponent(String(value))}`;
            }
            return targetUrl;
        };
        const ep = { abandonedCart: 'carts/abandoned', order: 'orders', product: 'products', customer: 'customers', coupon: 'coupons', brand: 'brands', category: 'categories', feedback: 'feedbacks' };
        const idF = { abandonedCart: 'abandonedCartId', order: 'orderId', product: 'productId', customer: 'customerId', coupon: 'couponId', brand: 'brandId', category: 'categoryId' };
        for (let i = 0; i < items.length; i++) {
            try {
                ...
                if (resource === 'productVariant' && operation === 'getAll') {
                    ...
                    if (useJson) {
                        ...
                        if (!parsedBody || typeof parsedBody !== 'object' || Array.isArray(parsedBody) || Object.keys(parsedBody).length === 0)
                            throw new n8n_workflow_1.NodeOperationError(this.getNode(), 'Variant JSON must be a non-empty object with at least one field to update.', { itemIndex: i });
                        body = asDataObject(parsedBody);
                    }
                    else {
                        body = { ...asNodeParameters(this.getNodeParameter('variantUpdateFields', i, {})) };
                        const branchCollection = asNodeParameters(this.getNodeParameter('variantBranchQuantities', i, {}));
                        ...
                        if (branches.length > 0) {
                            body.quantities = branches.map((entry, index) => {
                                ...
                                if (!entry.branch || !Number.isFinite(quantity) || quantity < 0)
                                    throw new n8n_workflow_1.NodeOperationError(this.getNode(), `Branch quantity ${index + 1} requires a branch ID and a quantity greater than or equal to 0.`, { itemIndex: i });
                                const out = { branch: entry.branch, quantity };
                                if (entry.reason_id)
                        ...
                        if (Object.keys(body).length === 0)
                            throw new n8n_workflow_1.NodeOperationError(this.getNode(), 'Choose at least one variant field or branch quantity to update.', { itemIndex: i });
                    }
                }
                    ...
                    if (useJson) {
                        ...
                        if (!raw || typeof raw !== 'object' || Array.isArray(raw))
                            throw new n8n_workflow_1.NodeOperationError(this.getNode(), 'Quantity JSON must be an object.', { itemIndex: i });
                        entry = { ...asNodeParameters(raw) };
                    }
                    else {
                        const extra = asNodeParameters(this.getNodeParameter('variantQuantityAdditionalFields', i, {}));
                        entry = {
                    ...
                    if (useJson) {
                        ...
                        const collection = asNodeParameters(this.getNodeParameter('inventoryItems', i, {}));
                    ...
                    if (resource === 'abandonedCart') {
                        const filters = asNodeParameters(this.getNodeParameter('abandonedCartFilters', i, {}));
                        if (filters.keyword)
                            url += `&keyword=${encodeURIComponent(String(filters.keyword))}`;
                    }
                    if (resource === 'order') {
                        const filters = asNodeParameters(this.getNodeParameter('orderFilters', i, {}));
                        url = appendFilters(url, filters, ['keyword', 'reference_id', 'status', 'payment_method', 'customer_id', 'from_date', 'to_date'], i);
                    }
                    if (resource === 'customer') {
                        const filters = asNodeParameters(this.getNodeParameter('customerFilters', i, {}));
                        url = appendFilters(url, filters, ['keyword', 'date_from', 'date_to'], i);
                    }
                    if (resource === 'coupon') {
                        const filters = asNodeParameters(this.getNodeParameter('couponFilters', i, {}));
                        url = appendFilters(url, filters, ['keyword', 'creation_date', 'expiration_date'], i);
                    }
                    if (resource === 'brand') {
                        const filters = asNodeParameters(this.getNodeParameter('brandFilters', i, {}));
                        url = appendFilters(url, filters, ['keyword'], i);
                    }
                    if (resource === 'category') {
                        const filters = asNodeParameters(this.getNodeParameter('categoryFilters', i, {}));
                        url = appendFilters(url, filters, ['keyword', 'status'], i);
                    }
                    if (resource === 'product') {
                        const filters = asNodeParameters(this.getNodeParameter('productFilters', i, {}));
                        if (filters.keyword)
                            url += `&keyword=${encodeURIComponent(String(filters.keyword))}`;
                        if (filters.status)
                    ...
                    if (resource === 'feedback') {
                        const filters = asNodeParameters(this.getNodeParameter('feedbackFilters', i, {}));
                        ...
                        for (const t of typeQueries) {
                            ...
                            const first = await sallaRequest({ method: 'GET', url: pageUrl, headers: hdr }, { itemIndex: i, readContext: 'feedback-fanout' });
                            ...
                            if (returnAll) {
                                const maxPages = 500;
                                const firstPagination = asDataObject((_a = first.pagination) !== null && _a !== void 0 ? _a : {});
                                ...
                                while (currentPage < totalPages && currentPage < maxPages) {
                                    ...
                                    const nextR = await sallaRequest({ method: 'GET', url: nextUrl, headers: hdr }, { itemIndex: i, readContext: 'pagination' });
                                    if (Array.isArray(nextR === null || nextR === void 0 ? void 0 : nextR.data) && nextR.data.length > 0)
                                        combined.push(...nextR.data);
                                    else
                                        break;
                                }
                                if (currentPage >= maxPages && totalPages > maxPages) {
                                    throw new n8n_workflow_1.NodeOperationError(this.getNode(), `Feedback Return All stopped at the 500-page safety limit for type "${t || 'all'}". Add date or type filters.`, { itemIndex: i });
                                }
                            }
                        }
                        const seenFeedbackIds = new Set();
                        const unique = combined.filter((entry) => {
                            if ((entry === null || entry === void 0 ? void 0 : entry.id) === undefined || (entry === null || entry === void 0 ? void 0 : entry.id) === null)
                                return true;
                    ...
                    if (useMultiple) {
                        ...
                        if (orderIds.length === 0) {
                            throw new n8n_workflow_1.NodeOperationError(this.getNode(), 'Select at least one order to cancel.', { itemIndex: i });
                        }
                        const results = [];
                        for (const orderId of orderIds) {
                            try {
                                ...
                                results.push({ order_id: orderId, success: true, data: (response === null || response === void 0 ? void 0 : response.data) || response });
                            }
                            catch (error) {
                                results.push({ order_id: orderId, success: false, error: (error === null || error === void 0 ? void 0 : error.message) || 'Unknown error' });
                            }
                        }
                        r = {
                            data: {
                    ...
                    if (useMultiple) {
                        const orderIds = [...new Set(csvToArray(this.getNodeParameter('orderIds', i, []), false))];
                        if (orderIds.length === 0) {
                            throw new n8n_workflow_1.NodeOperationError(this.getNode(), 'Select at least one order to update.', { itemIndex: i });
                        }
                        const results = [];
                        for (const orderId of orderIds) {
                            try {
                    ...
                    if (useJson) {
                        ...
                        if (typeof raw === 'string') {
                            try {
                                ...
                                throw new n8n_workflow_1.NodeOperationError(this.getNode(), 'Options JSON is not valid JSON. If you are using an n8n expression, make sure it starts with "=" (e.g. "={{ [...] }}").', { itemIndex: i });
                            }
                        }
                        else {
                            parsed = raw;
                        ...
                        const optList = asNodeParameters(this.getNodeParameter('optionsList', i, {}));
                        ...
                        if (optionEntries.length > 0) {
                            optionsToAdd = optionEntries.map((o) => {
                                ...
                                const valuesCollection = asNodeParameters((_a = o.values) !== null && _a !== void 0 ? _a : {});
                                ...
                                if (valueEntries.length > 0) {
                                    opt.values = valueEntries.map((v) => {
                                        const val = { name: v.name, price: v.price || 0 };
                                        if (v.quantity)
                                            val.quantity = v.quantity;
                                        if (v.display_value)
                                            val.display_value = v.display_value;
                                        if (v.is_default)
                                            val.is_default = v.is_default;
                                        return val;
                                    });
                                }
                                return opt;
                    ...
                    if (optionsToAdd.length === 0) {
                        throw new n8n_workflow_1.NodeOperationError(this.getNode(), 'No options to create. Add at least one entry in "Options" or provide a non-empty JSON array.', { itemIndex: i });
                    }
                    const optResults = [];
                    for (const optBody of optionsToAdd) {
                        const optResp = await sallaRequest({ method: 'POST', url: optUrl, headers: hdr, body: optBody }, { itemIndex: i, ctx: `productOption/create name=${optBody.name}` });
                    ...
                    const productId = this.getNodeParameter('productId', i);
                    const prodResp = await sallaRequest({ method: 'GET', url: `${API}/api/v1/salla/products/${productId}`, headers: hdr }, { itemIndex: i, ctx: `productOption/getAll product=${productId}` });
                    const productData = asDataObject((_b = prodResp.data) !== null && _b !== void 0 ? _b : {});
                    ...
                    if (useJson) {
                        ...
                        if (typeof raw === 'string') {
                            try {
                                ...
                                throw new n8n_workflow_1.NodeOperationError(this.getNode(), 'Option IDs JSON is not valid JSON. If using an n8n expression, make sure it starts with "=".', { itemIndex: i });
                            }
                        }
                        else {
                            parsed = raw;
                        ...
                        for (const e of entries) {
                            const id = typeof e === 'object' ? e === null || e === void 0 ? void 0 : e.id : e;
                            if (id === undefined || id === null || id === '')
                                throw new n8n_workflow_1.NodeOperationError(this.getNode(), 'Each JSON entry must be an ID or an object with an "id" field.', { itemIndex: i });
                    ...
                    for (const optionId of idsToDelete) {
                        try {
                            await sallaRequest({ method: 'DELETE', url: `${API}/api/v1/salla/products/options/${optionId}`, headers: hdr }, { itemIndex: i, ctx: `delete option id=${optionId}` });
                            ...
                            if (requestError.description) {
                                try {
                                    entry.field_errors = JSON.parse(requestError.description);
                                }
                                catch {
                                    entry.field_errors = requestError.description;
                                }
                            }
                            results.push(entry);
                        }
                    ...
                    if (!bulkMode) {
                        const r0 = results[0];
                        if (!r0.success)
                            throw new n8n_workflow_1.NodeOperationError(this.getNode(), String(r0.error), { itemIndex: i });
                        ret.push({
                    ...
                    if (useJson) {
                        ...
                        if (typeof raw === 'string') {
                            try {
                                ...
                                throw new n8n_workflow_1.NodeOperationError(this.getNode(), 'Value JSON is not valid JSON. If using an n8n expression, make sure it starts with "=".', { itemIndex: i });
                            }
                        }
                        else {
                            parsed = raw;
                        ...
                        for (const rawEntry of entries) {
                            ...
                            if (!rawEntry || entry.id === undefined || entry.id === null || entry.id === '')
                                throw new n8n_workflow_1.NodeOperationError(this.getNode(), 'Each JSON entry must include an "id" field (the value ID to update).', { itemIndex: i });
                        ...
                        if (newPrice !== '' && newPrice !== null) {
                            ...
                            if (Number.isNaN(p))
                                throw new n8n_workflow_1.NodeOperationError(this.getNode(), `Price must be a number (got "${newPrice}")`, { itemIndex: i });
                            overrides.price = p;
                        }
                        if (newQty !== '' && newQty !== null) {
                            const q = Number(newQty);
                            if (Number.isNaN(q))
                                throw new n8n_workflow_1.NodeOperationError(this.getNode(), `Quantity must be a number (got "${newQty}")`, { itemIndex: i });
                            overrides.quantity = q;
                        }
                        if (newDisplay !== '')
                            overrides.display_value = newDisplay;
                        if (newIsDefault === 'true')
                            overrides.is_default = true;
                        else if (newIsDefault === 'false')
                            overrides.is_default = false;
                        if (Object.keys(overrides).length === 0)
                            throw new n8n_workflow_1.NodeOperationError(this.getNode(), 'Nothing to update — fill at least one field.', { itemIndex: i });
                        updatesToApply.push({ valueId: String(valueId), overrides });
                    }
                    const results = [];
                    for (const { valueId, overrides } of updatesToApply) {
                        try {
                            const curResp = await sallaRequest({ method: 'GET', url: `${API}/api/v1/salla/products/options/values/${valueId}`, headers: hdr }, { itemIndex: i, ctx: `updateValue GET valueId=${valueId}` });
                            const cur = asDataObject((_c = curResp.data) !== null && _c !== void 0 ? _c : {});
                            const currentPrice = asDataObject((_d = cur.price) !== null && _d !== void 0 ? _d : {});
                            const merged = {
                                name: cur.name,
                                price: (_f = (_e = currentPrice.amount) !== null && _e !== void 0 ? _e : cur.price) !== null && _f !== void 0 ? _f : 0,
                            ...
                            const putResp = await sallaRequest({ method: 'PUT', url: `${API}/api/v1/salla/products/options/values/${valueId}`, headers: hdr, body: merged }, { itemIndex: i, ctx: `updateValue PUT valueId=${valueId}` });
                            results.push({ id: valueId, success: true, data: (putResp === null || putResp === void 0 ? void 0 : putResp.data) || putResp });
                        }
                        catch (err) {
                            const requestError = asRequestError(err);
                            const entry = {
                    ...
                    const buildFormOverrides = () => {
                        ...
                        if (type)
                            overrides.type = type;
                        if (displayType)
                            overrides.display_type = displayType;
                        if (required === 'true')
                            overrides.required = true;
                        else if (required === 'false')
                            overrides.required = false;
                        if (replaceValues) {
                            const valColl = asNodeParameters(this.getNodeParameter('updateOptionValues', i, {}));
                            const vals = Array.isArray(valColl.value)
                                ? asNodeParameterArray(valColl.value)
                                : [];
                            overrides.values = vals.map((v) => {
                                const out = { name: v.name, price: v.price || 0 };
                                if (v.quantity)
                                    out.quantity = v.quantity;
                                if (v.display_value)
                                    out.display_value = v.display_value;
                                if (v.is_default)
                                    out.is_default = v.is_default;
                                return out;
                            });
                        }
                        return overrides;
                    ...
                    if (useJson) {
                        ...
                        if (typeof raw === 'string') {
                            try {
                                ...
                                throw new n8n_workflow_1.NodeOperationError(this.getNode(), 'Option JSON is not valid JSON. If using an n8n expression, make sure it starts with "=".', { itemIndex: i });
                            }
                        }
                        else {
                            parsed = raw;
                        ...
                        if (Array.isArray(parsed)) {
                            ...
                            for (const rawEntry of parsed) {
                                const entry = asDataObject(rawEntry);
                                if (!rawEntry || entry.id === undefined || entry.id === null || entry.id === '')
                                    throw new n8n_workflow_1.NodeOperationError(this.getNode(), 'Each bulk JSON entry must include an "id" field (the option ID to update).', { itemIndex: i });
                                const { id, ...overrides } = entry;
                    ...
                    for (const { optionId, overrides } of updatesToApply) {
                        try {
                            const curResp = await sallaRequest({ method: 'GET', url: `${API}/api/v1/salla/products/options/${optionId}`, headers: hdr }, { itemIndex: i, ctx: `update option GET optionId=${optionId}` });
                            const cur = asDataObject((_g = curResp.data) !== null && _g !== void 0 ? _g : {});
                            const curWritable = { ...cur };
                            ...
                            const preservedValues = Array.isArray(cur.values)
                                ? cur.values.map((v) => {
                                    var _a, _b, _c;
                                    const valuePrice = asDataObject((_a = v.price) !== null && _a !== void 0 ? _a : {});
                                    return {
                                        id: v.id,
                                        name: v.name,
                                        price: (_c = (_b = valuePrice.amount) !== null && _b !== void 0 ? _b : v.price) !== null && _c !== void 0 ? _c : 0,
                                ...
                                : [];
                            const merged = { ...curWritable, values: preservedValues, ...overrides };
                            const putResp = await sallaRequest({ method: 'PUT', url: `${API}/api/v1/salla/products/options/${optionId}`, headers: hdr, body: merged }, { itemIndex: i, ctx: `update option PUT optionId=${optionId}` });
                            results.push({ id: optionId, success: true, data: (putResp === null || putResp === void 0 ? void 0 : putResp.data) || putResp });
                        }
                        catch (err) {
                            const requestError = asRequestError(err);
                            const entry = {
                    ...
                    if (!bulkMode) {
                        ...
                        if (!r0.success)
                            throw new n8n_workflow_1.NodeOperationError(this.getNode(), String(r0.error), { itemIndex: i });
                        ret.push({ json: asDataObject((_h = r0.data) !== null && _h !== void 0 ? _h : {}), pairedItem: { item: i } });
                    }
                    else {
                        ret.push({
                            json: {
                    ...
                    if (useCustom) {
                        ...
                        if (!hasFields(body)) {
                            throw new n8n_workflow_1.NodeOperationError(this.getNode(), 'Advanced JSON Body must contain at least one field.', { itemIndex: i });
                        }
                        // ── Order Create ──
                    }
                        ...
                        if (!Number.isSafeInteger(customerIdNumber) || customerIdNumber <= 0) {
                            throw new n8n_workflow_1.NodeOperationError(this.getNode(), 'Customer is required and must contain a valid numeric Salla ID.', { itemIndex: i });
                        }
                        const buildProductLine = (productIdValue, quantityValue, selectedOptionsValue, lineNumber) => {
                            ...
                            if (!Number.isSafeInteger(productIdNumber) || productIdNumber <= 0) {
                                throw new n8n_workflow_1.NodeOperationError(this.getNode(), `Product is required and must contain a valid numeric Salla ID (line ${lineNumber}).`, { itemIndex: i });
                            }
                            const qty = Number(quantityValue);
                            if (!Number.isFinite(qty) || qty < 1) {
                                throw new n8n_workflow_1.NodeOperationError(this.getNode(), `Product ${lineNumber} quantity must be a number of 1 or more.`, { itemIndex: i });
                            }
                            const product = { identifier_type: 'id', identifier: productIdNumber, quantity: qty };
                            ...
                            for (const sel of selectedOptions) {
                                ...
                                if (!pId || !optId || !valId || unexpected.length > 0
                                    ...
                                    throw new n8n_workflow_1.NodeOperationError(this.getNode(), `Product ${lineNumber} option "${sel}" is malformed. Select the option again.`, { itemIndex: i });
                                }
                                if (pId !== String(productId)) {
                                    throw new n8n_workflow_1.NodeOperationError(this.getNode(), `Product ${lineNumber} option "${sel}" belongs to product ${pId}, but this line uses product ${productId}.`, { itemIndex: i });
                                }
                                if (!optionsMap[optId])
                                    optionsMap[optId] = [];
                        ...
                        if (useMultipleProducts) {
                            const collection = asNodeParameters(this.getNodeParameter('orderProducts', i, {}));
                            const productLines = Array.isArray(collection.product)
                                ? asNodeParameterArray(collection.product)
                                : [];
                            if (productLines.length === 0) {
                                throw new n8n_workflow_1.NodeOperationError(this.getNode(), 'Add at least one product line to the order.', { itemIndex: i });
                            }
                            products = productLines.map((line, index) => buildProductLine(line.productId, line.quantity, line.options || [], index + 1));
                        }
                        else {
                            products = [buildProductLine(this.getNodeParameter('orderProductId', i), this.getNodeParameter('orderProductQty', i), this.getNodeParameter('orderProductOptions', i, []), 1)];
                        }
                        const extra = asNodeParameters(this.getNodeParameter('orderAdditionalFields', i, {}));
                        ...
                        const payment = { status: payStatus };
                        if (payStatus === 'paid') {
                            const payMethod = this.getNodeParameter('orderPaymentMethod', i);
                            if (!payMethod) {
                                throw new n8n_workflow_1.NodeOperationError(this.getNode(), 'Payment Method is required when Payment Status is Paid.', { itemIndex: i });
                            }
                            ...
                            if (!Array.isArray(accepted) || accepted.length === 0) {
                                throw new n8n_workflow_1.NodeOperationError(this.getNode(), 'Select at least one Accepted Payment Method for a pending-payment order.', { itemIndex: i });
                            }
                            payment.accepted_methods = accepted;
                        }
                        else {
                            throw new n8n_workflow_1.NodeOperationError(this.getNode(), `Unsupported Payment Status "${payStatus}".`, { itemIndex: i });
                        }
                        body = {
                        ...
                        if (deliveryMethod === 'pickup' && !extra.branch_id) {
                            throw new n8n_workflow_1.NodeOperationError(this.getNode(), 'Branch is required when Delivery Method is Pickup.', { itemIndex: i });
                        }
                        if (deliveryMethod === 'shipping') {
                            ...
                            if (!Number.isSafeInteger(courierId) || courierId <= 0) {
                                throw new n8n_workflow_1.NodeOperationError(this.getNode(), 'Courier is required when Delivery Method is Shipping.', { itemIndex: i });
                            }
                            ...
                            if (missingAddressFields.length > 0) {
                                throw new n8n_workflow_1.NodeOperationError(this.getNode(), `Shipping Address is missing: ${missingAddressFields.join(', ')}.`, { itemIndex: i });
                            }
                            for (const field of ['country', 'city', 'district']) {
                                if (shipTo[field] !== undefined && shipTo[field] !== '') {
                                    const value = Number(shipTo[field]);
                                    if (!Number.isSafeInteger(value) || value <= 0) {
                                        throw new n8n_workflow_1.NodeOperationError(this.getNode(), `Shipping Address ${field} must be a valid numeric Salla ID.`, { itemIndex: i });
                                    }
                                    shipTo[field] = value;
                                }
                            }
                            ...
                            if (hasLatitude !== hasLongitude) {
                                throw new n8n_workflow_1.NodeOperationError(this.getNode(), 'Shipping Address requires both Latitude and Longitude when either is provided.', { itemIndex: i });
                            }
                            if (hasLatitude && hasLongitude) {
                        ...
                        // Additional Fields
                        const extra = asNodeParameters(this.getNodeParameter('productAdditionalFields', i, {}));
                        ...
                        if (!/^\+\d{1,4}$/.test(countryCode)) {
                            throw new n8n_workflow_1.NodeOperationError(this.getNode(), 'Mobile Country Code must start with + and contain 1–4 digits, for example +966.', { itemIndex: i });
                        }
                        body = {
                        ...
                        if (email)
                            body.email = email;
                        const extra = asNodeParameters(this.getNodeParameter('customerAdditionalFields', i, {}));
                        if (extra.gender)
                        ...
                        if (!Number.isFinite(amount) || amount <= 0) {
                            throw new n8n_workflow_1.NodeOperationError(this.getNode(), 'Discount Amount must be greater than 0.', { itemIndex: i });
                        }
                        if (couponType === 'percentage' && amount > 100) {
                            throw new n8n_workflow_1.NodeOperationError(this.getNode(), 'Percentage discounts cannot be greater than 100.', { itemIndex: i });
                        }
                        body = {
                            code,
                            type: couponType,
                        ...
                        if (sd) {
                            ...
                            if (sd > expiryDate) {
                                throw new n8n_workflow_1.NodeOperationError(this.getNode(), 'Start Date cannot be after Expiry Date.', { itemIndex: i });
                            }
                            body.start_date = sd;
                        }
                        ...
                        const extra = asNodeParameters(this.getNodeParameter('couponAdditionalFields', i, {}));
                        ...
                        if (Object.prototype.hasOwnProperty.call(fields, 'amount')) {
                            ...
                            if (!Number.isFinite(amount) || amount <= 0) {
                                throw new n8n_workflow_1.NodeOperationError(this.getNode(), 'Coupon Amount must be greater than 0.', { itemIndex: i });
                            }
                        ...
                        if (!/^https?:\/\//i.test(logoUrl)) {
                            throw new n8n_workflow_1.NodeOperationError(this.getNode(), 'Logo URL must be an absolute http:// or https:// image URL.', { itemIndex: i });
                        }
                        body = {
                            name,
                            logo_url: logoUrl,
                        };
                        const extra = asNodeParameters(this.getNodeParameter('brandAdditionalFields', i, {}));
                        ...
                        const extra = asNodeParameters(this.getNodeParameter('categoryAdditionalFields', i, {}));
                        if (extra.parent_id)
                    ...
                    if (!useCustom && operation === 'update' && ['customer', 'coupon', 'brand', 'category'].includes(resource) && !hasFields(body)) {
                        throw new n8n_workflow_1.NodeOperationError(this.getNode(), `Add at least one ${resource} field to update, or turn on Use Advanced JSON.`, { itemIndex: i });
                    }
                    if (!url) {
                        if (operation === 'create') {
                ...
                if (resource === 'customApiCall') {
                    ...
                    let endpoint = this.getNodeParameter('customEndpoint', i).trim();
                    if (!endpoint) {
                        throw new n8n_workflow_1.NodeOperationError(this.getNode(), 'Endpoint is required, for example /orders or /customers/123.', { itemIndex: i });
                    }
                    if (/^https?:\/\//i.test(endpoint)) {
                        throw new n8n_workflow_1.NodeOperationError(this.getNode(), 'Endpoint must be a Salla path such as /orders, not a full URL.', { itemIndex: i });
                    }
                    if (endpoint.includes('?')) {
                        throw new n8n_workflow_1.NodeOperationError(this.getNode(), 'Put query parameters in Query Parameters, not inside Endpoint.', { itemIndex: i });
                    }
                    if (!endpoint.startsWith('/'))
                        endpoint = '/' + endpoint;
                    const queryStr = this.getNodeParameter('customQuery', i, '').trim();
                ...
                if (!url)
                    throw new n8n_workflow_1.NodeOperationError(this.getNode(), `Unsupported: ${resource}/${operation}`, { itemIndex: i });
                const opts = { method, url, headers: hdr };
                if (body)
                    opts.body = body;
                if (!preFetched) {
                ...
                if (!preFetched && operation === 'getAll' && this.getNodeParameter('returnAll', i, false) === true && Array.isArray(response.data)) {
                    const maxPages = 500;
                    const pagination = asDataObject((_j = response.pagination) !== null && _j !== void 0 ? _j : {});
                    ...
                    while (currentPage < totalPages && currentPage < maxPages) {
                        ...
                        const nextR = await sallaRequest({ ...opts, url: nextUrl }, { itemIndex: i, readContext: 'pagination' });
                        if (Array.isArray(nextR.data) && nextR.data.length > 0) {
                            collected = collected.concat(nextR.data);
                    ...
                    if (currentPage >= maxPages && totalPages > maxPages) {
                        throw new n8n_workflow_1.NodeOperationError(this.getNode(), `Return All stopped at the 500-page safety limit (${response.data.length} results collected, ${totalPages} pages reported). Add filters or use Page mode.`, { itemIndex: i });
                    }
                }
                // After product creation, upload image as file if image_url was provided
                const responseData = asDataObject((_k = response.data) !== null && _k !== void 0 ? _k : {});
                if (resource === 'product' && operation === 'create' && responseData.id) {
                    const extraFields = asNodeParameters(this.getNodeParameter('productAdditionalFields', i, {}));
                    if (extraFields.image_url) {
                        try {
                            ...
                            this.logger.warn(`Image upload failed after product ${responseData.id} was created`, { error: imgErr });
                        }
                    }
                }
                ...
                if (e instanceof n8n_workflow_1.NodeOperationError) {
                    ...
                    const sallaResp = ((_l = e === null || e === void 0 ? void 0 : e.response) === null || _l === void 0 ? void 0 : _l.data) || ((_o = (_m = e === null || e === void 0 ? void 0 : e.cause) === null || _m === void 0 ? void 0 : _m.response) === null || _o === void 0 ? void 0 : _o.data) || (e === null || e === void 0 ? void 0 : e.body);
                    cleanMsg = (e === null || e === void 0 ? void 0 : e.message) || 'Unknown error';
                    if (sallaResp) {
                        const d = typeof sallaResp === 'string'
                            ? (() => { try {
                                return JSON.parse(sallaResp);
                            }
                            catch {
                                return { raw: sallaResp };
                            } })()
                            : sallaResp;
                        const eObj = d === null || d === void 0 ? void 0 : d.error;
                        if (eObj && typeof eObj === 'object')
                            cleanMsg = eObj.message || cleanMsg;
                        else if (typeof eObj === 'string')
                            cleanMsg = (d === null || d === void 0 ? void 0 : d.message) || eObj;
                        else if (d === null || d === void 0 ? void 0 : d.message)
                            cleanMsg = d.message;
                    }
                    const status = ((_p = e === null || e === void 0 ? void 0 : e.response) === null || _p === void 0 ? void 0 : _p.status) || (e === null || e === void 0 ? void 0 : e.statusCode);
                    if (status)
                        cleanMsg = `[${status}] ${cleanMsg}`;
                }
                if (this.continueOnFail()) {
                    ret.push({ json: { error: cleanMsg }, pairedItem: { item: i } });
                    continue;
                }
                throw new n8n_workflow_1.NodeOperationError(this.getNode(), cleanMsg, { itemIndex: i });
            }
        }
        return [ret];
    }
}
exports.SallaFlow = SallaFlow;
//# sourceMappingURL=SallaFlow.node.js.map