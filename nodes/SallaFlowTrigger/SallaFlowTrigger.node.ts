/*
 * Event groups mirror Salla's catalogue and the order exposed by prior
 * SallaFlow trigger versions, including the saved-workflow compatibility alias.
 */
...
export class SallaFlowTrigger implements INodeType {
      ...
      async checkExists(this: IHookFunctions): Promise<boolean> {
        ...
        try {
          ...
          const subscriptions = Array.isArray(res.subscriptions)
            ? res.subscriptions as IDataObject[]
            : [];
          if (res.success) {
      ...
      async create(this: IHookFunctions): Promise<boolean> {
        ...
        if (!res.success) {
          throw new NodeOperationError(
            this.getNode(),
            `Failed to register webhook: ${JSON.stringify(res)}`,
          );
        }
        return true;
      },
